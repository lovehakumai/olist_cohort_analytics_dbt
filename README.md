...
### Purpose
In General, Ecommerce business owners want to see the customer retention to make segments of customers.
And E-commerce data is fundamentally composed of 3 relational datasets, Order / Customer / Items.
The cohort analytics drives strategic customer segmentation.

### About Cohort
The basic but proven analytics method for this kind of business.
...
* 3 Layers
    ...
    * Intermediate(fact/dim) : Create the base models for creating mart with complicated transfomations
    * Mart : Create the Table for Analytics user from Intermediate Models
...
![dbt Data Lineage DAG](assets/dbt_lineage.png)

## 3. Analytics Engineering Highlights
### Ensuring Idempotency of Result 
As you can see above, the models are transformed several times.  
CTE is known as useful way to develop the pipeline but dbt can't run the tests in CTE parts.  
This structure helps project ensures idempotency.

### Automated Building Process with Github Actions Workflows  
This pipeline doesn't use dbt cloud for running automatically,It uses Github Actions instead of it.  
※dbt cloud: UI application with user-friendly platform for developing and managing dbt project.   

Github Actions is One of the Github Services which helps us to triggar dbt commands automatically.  
* The points : 
...
This is dbt's standard feature, When you change code in outside of main branch  
...
You can push or merge it after checking the new code works as you expected in snowflake.  

### Incremental Feature:
To ensure scalability, dbt models uses incremental option.  
```config(materialized =  'incremental')```.  

If the tables already exists, It will be converted into ```MERGE``` command in Snowflake(update or insert).  
...
### Generic Test
Each model includes specific grain and its unique column (or columns ).
dbt(and its library dbt-utils) has standard test algorithm for checking if the unique key is truly unique one or not.
